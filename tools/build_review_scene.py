...
def build(ply,
    ...
    # ---- the splat node ----------------------------------------------------
    shape = cmds.createNode("gaussianSplatNode",
                            name=f"{prefix}_splatShape",
                            parent=shift)
    ...
    bcentre = np.array([(lo[0] + hi[0]) / 2.0, (base + top) / 2.0,
                        (lo[2] + hi[2]) / 2.0])
    print("[review] cull box (levelled world): base Y %+.4f  top Y %+.4f" %
          (base, top))

    cube = cmds.polyCube(name=f"{prefix}_cullBox", w=1, h=1, d=1, ch=False)[0]
    ...
    _wire(cube)
    cmds.connectAttr(cube + ".worldMatrix[0]",
                     shape + ".cullBoxMatrix",
                     force=True)
    ...
    made = []
    if s.get('found'):
        loc = cmds.spaceLocator(name=f"{prefix}_sphereCentre")[0]
        _adopt(loc, shift, t=s['centre'])
        for ax in "XYZ":
            cmds.setAttr(loc + ".localScale" + ax, s['radius'] * 0.05)
        sph = cmds.polySphere(name=f"{prefix}_shellFit",
    ...
    core = pts[np.linalg.norm(pts -
                              np.median(pts, axis=0), axis=1) <= s['cut']]
    ...
    print("\n[review] rig check")
    print(
        "  scene 'up' maps to      : [%+.4f %+.4f %+.4f]  (%.4f deg off world +Y)"
        % (*up4, tilt))
    print("  ground sits at Y        : %+.4f  (densest height slab)" %
          ground_y)
    print("  splat heights p50 / p99 : %+.4f / %+.4f" %
          (np.percentile(w[:, 1], 50), np.percentile(w[:, 1], 99)))
    ...
    shader_local = (np.hstack([core, np.ones(
        (len(core), 1))]) @ (m @ inv))[:, :3]
    ...
    print("  -> %s" %
          ("LEVELLED, UPRIGHT, BOX ALIGNED" if ok else "RIG IS WRONG"))
    ...
    if ref_size:
        ...
        cmds.setAttr(ref + ".translateY",
                     float(ref_size[1]) / 2.0)  # stand it on the ground
        _wire(ref)
    ...
    print("[review] at scale %g (cm per PLY unit) the cropped scene measures "
          "%.2f x %.2f x %.2f m" % (scale, bsize[0] * scale / 100.0, bsize[1] *
                                    scale / 100.0, bsize[2] * scale / 100.0))
...
if __name__ == '__main__':
    ...
    ap.add_argument('--prefix',
                    type=str,
                    required=True,
                    help='prefix for naming the nodes')
    ap.add_argument('--plugin', required=True)
    ap.add_argument('--percent',
                    type=float,
                    default=10.0,
                    help='fraction of splats used for the analysis')
    ap.add_argument('--display-percent',
                    type=float,
                    default=100.0,
